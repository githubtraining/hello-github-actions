...
- `uses: actions/checkout@v1` uses a community action called [`checkout`](https://github.com/actions/checkout) to allow the workflow to access the contents of our repository
- `uses: ./action-a` provides the relative path the action we've created in the `action-a` directory of our repository
...
This action will run anytime a new commit is created or pushed to the remote repository. Since you just created a commit, the workflow should have been triggered. This might take a few minutes since it's the first time running in this repository.
...
> Note: This course will only work for members of the GitHub Actions beta. If you're not in the GitHub Actions beta, you won't receive a response from the action.
